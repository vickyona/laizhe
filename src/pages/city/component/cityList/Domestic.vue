<template>
    ...
    <ul class="right-sidebar" id="position" >
      <li v-for="(items, index) in cities" class="city-letter" :key="index" @touchstart="handleTouchstart" @touchmove="handleTouchstart">{{index}}</li>
    </ul>
  </div>
</template>
...
export default {
  computed: {
  	positionCity() {
  		return this.$store.state.domestic.positionCity;
  	},
    hotCity() {
     	return this.$store.state.domestic.hotCity;
    },
    cities() {
    	return this.$store.state.domestic.cities;
    }
  },
  methods: {
